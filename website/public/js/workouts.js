  ...
  request.onload = function () {
    ...
    if (request.status >= 200 && request.status < 400) {
      data.results.forEach(exercise => {
          ...
          inp1.type = "hidden";
          inp1.name = "exercise_name"
          inp1.value = exercise.name;

          inp2.type = "date";
          inp2.name = "workout_day";
          inp2.value = "";

          inp3.type = "time";
          inp3.name = "start_time";
          inp3.value = "";
          ...
          inp5.type = "hidden";
          inp5.name = "currTime"
          inp5.value = new Date().toString();

          form.style = "text-align: center"
          form.method = "POST";
          form.action = "/btn_wkt_add";
...
function searchExercise2() {
  searchInputTxt = document.getElementById('wo-searchBar').value.trim();
  var request2 = new XMLHttpRequest();
  request2.open('GET', 'https://wger.de/api/v2/exerciseinfo/?format=json&language=2&limit=100&offset=100', true);
  request2.onload = function () {
    // Begin accessing JSON data here
    var data2 = JSON.parse(this.response);
    if (request2.status >= 200 && request2.status < 400) {
      data2.results.forEach(exercise => {
          const card = document.createElement('div');
          card.setAttribute('class', 'card');
          const img = document.createElement('img');
          const h2 = document.createElement('h2');
